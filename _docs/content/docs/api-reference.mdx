...
Values live in the request's own `context.Context`, keyed by an unexported key
type. They therefore survive any `r.WithContext(...)` performed by a downstream
middleware, and are garbage-collected with the request.
...
func Delete(r *http.Request) // retained no-op; nothing needs freeing
